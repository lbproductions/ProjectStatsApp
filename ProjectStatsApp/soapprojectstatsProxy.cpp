...
int projectstatsProxy::liveGameList(const char *endpoint, const char *soap_action, LiveGameList &result)
{	struct soap *soap = this;
	...
	soap_begin(soap);
	soap_serializeheader(soap);
	soap_serialize_ps__liveGameList(soap, &soap_tmp_ps__liveGameList);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put_ps__liveGameList(soap, &soap_tmp_ps__liveGameList, "ps:liveGameList", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	}
	...
	if (soap_begin_recv(soap)
	 ...
	 || soap_body_begin_in(soap))
		return soap_closesock(soap);
	soap_tmp_ps__liveGameListResponse = soap_get_ps__liveGameListResponse(soap, NULL, "ps:liveGameListResponse", "");
	if (soap->error)
		return soap_recv_fault(soap, 0);
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap_closesock(soap);
	result = soap_tmp_ps__liveGameListResponse->result;
	return soap_closesock(soap);
}
